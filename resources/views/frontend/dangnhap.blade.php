				...
				<div class="card-body">
					...
					<form method="post" action="{{ route('login') }}">
						...
						<div class="mb-3">
							...
							<input type="text" class="form-control{{ ($errors->has('email'))  ? ' is-invalid' : '' }}" id="email" name="email" value="{{ old('email') }}" placeholder="nhập email của bạn" required />
							@if ($errors->has('email'))
								<span class="invalid-feedback" role="alert"><strong>{{ empty($errors->first('email')) ? : $errors->first('email') }}</strong></span>
							@endif
						</div>
						<div class="mb-3">
